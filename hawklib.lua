-- Image Function is firstly made by __spyro. After the ui libs with image function probably skidded this source
    ...
    local function ResizeMenu(resizer, mainFrame)
                ...
                if resizing then
                    ...
                    for _, child in pairs(mainFrame:GetChildren()) do
                        if child:IsA("GuiObject") then
                            local childAspectRatio = child.Size.X.Offset / child.Size.Y.Offset
                            child.Size = UDim2.new(
                                child.Size.X.Scale, 
                                newSize.X.Offset * (child.Size.X.Scale / mainFrame.Size.X.Scale), 
                                child.Size.Y.Scale, 
                                (newSize.X.Offset * (child.Size.X.Scale / mainFrame.Size.X.Scale)) / childAspectRatio
                            )
                        end
                    end
                end
...
function HawkLib:Window(Win)
	if _Hawk == "ohhahtuhthttouttpwuttuaunbotwo" then
        ...
        Resizer.Parent = Main
        Resizer.BackgroundColor3 = HawkLib.Themes[Theme].TitleBar
        Resizer.BorderSizePixel = 0
        Resizer.AnchorPoint = Vector2.new(1, 1)
        Resizer.Position = UDim2.new(1, 0, 1, 0)
        Resizer.Size = UDim2.new(0, 20, 0, 20)
    
        ResizerCorner.CornerRadius = UDim.new(0, 5)
        ...
        Main:GetPropertyChangedSignal("Size"):Connect(function()
            if not debounce then
                ...
                lastSize = Main.Size
                delay(5, function()
                    if lastSize == Main.Size then
                        local sizeData = game:GetService("HttpService"):JSONEncode({
