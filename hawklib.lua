...
ResizeTriangle.Name = "ResizeTriangle"
ResizeTriangle.Parent = Main
ResizeTriangle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
...
ResizeTriangle.Text = "◢"
ResizeTriangle.TextColor3 = Color3.fromRGB(200, 200, 200)
ResizeTriangle.TextSize = 20
ResizeTriangle.ZIndex = 1000
...
local lastResize = 0
local minSize = Vector2.new(400, 300)  -- Minimum size of the UI
local aspectRatio = Main.Size.X.Offset / Main.Size.Y.Offset

ResizeTriangle.MouseButton1Down:Connect(function()
    isResizing = true
    lastResize = tick()
end)
...
RunService.RenderStepped:Connect(function()
    if isResizing then
        ...
        local newSize = Vector2.new(
            math.max(mouse.X - Main.AbsolutePosition.X, minSize.X),
            math.max(mouse.Y - Main.AbsolutePosition.Y, minSize.Y)
        )
        
        -- Maintain aspect ratio
        if newSize.X / newSize.Y > aspectRatio then
            newSize = Vector2.new(newSize.X, newSize.X / aspectRatio)
        else
            newSize = Vector2.new(newSize.Y * aspectRatio, newSize.Y)
        end
        
        Main.Size = UDim2.new(0, newSize.X, 0, newSize.Y)
        
        -- Resize all other elements proportionally
        ResizeAllElements(Main, newSize)
        
        lastResize = tick()
    elseif tick() - lastResize > 5 and lastResize ~= 0 then
        SaveSizeToFile()
        lastResize = 0
    end
end)

function ResizeAllElements(parent, newSize)
    for _, child in ipairs(parent:GetDescendants()) do
        if child:IsA("GuiObject") and child ~= ResizeTriangle then
            local sizeX = child.Size.X.Scale * newSize.X + child.Size.X.Offset
            local sizeY = child.Size.Y.Scale * newSize.Y + child.Size.Y.Offset
            child.Size = UDim2.new(0, sizeX, 0, sizeY)
            
            local posX = child.Position.X.Scale * newSize.X + child.Position.X.Offset
            local posY = child.Position.Y.Scale * newSize.Y + child.Position.Y.Offset
            child.Position = UDim2.new(0, posX, 0, posY)
        end
    end
end

function SaveSizeToFile()
    local size = {
        width = Main.Size.X.Offset,
        height = Main.Size.Y.Offset
    }
    writefile("menu_size.json", game:GetService("HttpService"):JSONEncode(size))
end

local function LoadSizeFromFile()
    if isfile("menu_size.json") then
        local size = game:GetService("HttpService"):JSONDecode(readfile("menu_size.json"))
        Main.Size = UDim2.new(0, size.width, 0, size.height)
        ResizeAllElements(Main, Vector2.new(size.width, size.height))
    end
end    
