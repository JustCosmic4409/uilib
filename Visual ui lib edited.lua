...
function Library:CreateWindow(HubName, GameName, IntroText, IntroIcon, ImprovePerformance, ConfigFolder, Theme)
    ...
    function Tabs:CreateTab(TabName, DefaultVisibility, Icon, RectOffset, RectSize)
        ...
        function Sections:CreateSection(Name)
            ...
            function Elements:CreateDropdown(Name, List, Default, DebounceAmount, Callback, MultipleOptions)
                local Name = Name or 'Dropdown'
                local List = List or {}
                local Callback = Callback or function() end
                local DebounceAmount = DebounceAmount or 0.25
                local Default = Default or nil
                local MultipleOptions = MultipleOptions or false
                local SelectedItems = {}
                local Opened = false
                local Debounce = false
                    ...
                    OptionButton.MouseButton1Click:Connect(function()
                        task.spawn(function()
                            if MultipleOptions then
                                ...
                                DropdownSelectedText.Text = displayText
                            else
                            ...
                            Callback(MultipleOptions and SelectedItems or SelectedItems[1])
                            Config[Name] = MultipleOptions and SelectedItems or SelectedItems[1]
                ...
                if Default ~= nil then
                    if MultipleOptions and type(Default) == "table" then
                        for _, Item in ipairs(Default) do
                            local Option = DropList[Item..'OptionButton']
                            if Option then
                        ...
                        elseif #SelectedItems == 1 then
                            DropdownSelectedText.Text = SelectedItems[1]
                        else
                            DropdownSelectedText.Text = "Various"
                        end
                    elseif not MultipleOptions and type(Default) == "string" then
                ...
                function DropdownFunctions:Set(Value)
                    if MultipleOptions and type(Value) == "table" then
                        SelectedItems = Value
                    ...
                    elseif not MultipleOptions and type(Value) == "string" then
                        SelectedItems = {Value}
                        for _, Item in ipairs(List) do
                            local OptionButton = DropList[Item..'OptionButton']
                            if OptionButton then
                        ...
                        DropdownSelectedText.Text = Value
                    end
                    Callback(MultipleOptions and SelectedItems or SelectedItems[1])
                    Config[Name] = MultipleOptions and SelectedItems or SelectedItems[1]
                end
